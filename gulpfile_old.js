var gulp = require('gulp'), // Подключаем Gulp
	...
	include     = require('gulp-file-include') // соединение частей файла/файлов в родительском файле через инклуды
const
	workHtml = `main`

/* тут начинаются таски */
	/* task для сбора html */
	gulp.task('concat-html-all', function(){
		return gulp.src([
	...
	gulp.task('concat-html-work', function(){
		return gulp.src([
	...
	gulp.task('change-work-html', function(){
	...
	gulp.task('change-code', function(){
	    return gulp.src('src/assets/templates/*.html')
	    .pipe(browserSync.reload({ stream: true }))	// Обновляем CSS на странице при изменении	
	});
	gulp.task('index-code', gulp.series(
		'concat-html-work',
		'code'
	));	
	...
	gulp.task('watch', function() {
		...
		gulp.watch('src/assets/templates/*.html', gulp.parallel('index-code'));
	...
	// });
	/* 'all-scripts', */           
gulp.task('default', gulp.parallel('concat-html-work', 'css-libs', 'sass', 'all-scripts', 'browser-sync', 'watch'));
gulp.task('build', gulp.parallel('concat-html-all', 'es5', 'prebuild', 'clean', 'img', 'sass', 'all-scripts'));